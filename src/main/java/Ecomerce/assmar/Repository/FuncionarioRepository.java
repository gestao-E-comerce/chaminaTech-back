...
import Ecomerce.assmar.Entity.Funcionario;
...
public interface FuncionarioRepository extends JpaRepository<Funcionario, Long> {
    ...
    @Query("SELECT COUNT(v) > 0 FROM Venda v " + "WHERE v.funcionario.id = :funcionarioId " + "AND v.ativo = true")
    boolean existsByFuncionarioEmVendaAtiva(@Param("funcionarioId") Long funcionarioId);

    @Query("SELECT COUNT(c) > 0 FROM Caixa c " + "WHERE c.funcionario.id = :funcionarioId " + "AND c.ativo = true")
